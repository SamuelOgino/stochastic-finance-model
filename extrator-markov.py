...
def obter_dados_financeiros(ticker, start, end):
    ...
    # Se o yfinance retornar um MultiIndex (comum nas versões mais recentes)
    if isinstance(df.columns, pd.MultiIndex):
        # Achata as colunas pegando apenas o nível principal (Price)
        df.columns = df.columns.get_level_values(0)

    # Tenta usar 'Adj Close', se não existir, usa 'Close'
    if 'Adj Close' in df.columns:
        ...
        raise ValueError(
            f"Colunas inesperadas retornadas pelo yfinance: {df.columns}")
